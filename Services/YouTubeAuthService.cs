...
public sealed class YouTubeAuthService
{
    ...
    /// <summary>
    /// Browser OAuth wait limit. Closing the browser does not notify the local
    /// listener, so without a timeout the app would hang forever.
    /// </summary>
    ...
    /// Honours <paramref name="cancellationToken"/> so the UI can abort if the
    /// user closes the browser or cancels. Also times out if no callback arrives.
    /// </summary>
    public async Task<YouTubeService> AuthenticateAsync(CancellationToken cancellationToken = default)
    {
        ...
        var receiver = new CancellableLocalServerCodeReceiver();
        ...
        try
        {
            // WaitAsync unblocks even if HttpListener.GetContextAsync ignores Stop()
            // (common on macOS). The receiver also pings localhost to release the port.
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            throw new TimeoutException(
                "授權逾時（瀏覽器未在 5 分鐘內完成登入）。已取消等待，請再按「登入 / 授權」。");
        }
    ...
    public static bool HasStoredRefreshToken
    {
        get
        {
            var path = Path.Combine(TokenStoreDirectory, "Google.Apis.Auth.OAuth2.Responses.TokenResponse-user");
            return File.Exists(path);
        }
    }

    /// <summary>
    /// Restore a previous desktop OAuth session from the refresh token.
    /// Does not open a browser.
    /// </summary>
    public async Task<YouTubeService?> TryRestoreAsync(CancellationToken cancellationToken = default)
    {
        var secretsPath = FindClientSecretsPath();
        if (secretsPath is null || !HasStoredRefreshToken)
            return null;
        ...
        try
        {
            ...
            Directory.CreateDirectory(TokenStoreDirectory);
            var store = new FileDataStore(TokenStoreDirectory, fullPath: true);
            var token = await store.GetAsync<TokenResponse>(UserId);
            ...
            _credential = new UserCredential(flow, UserId, token);
            var refreshed = await _credential.RefreshTokenAsync(cancellationToken);
            if (!refreshed && string.IsNullOrWhiteSpace(_credential.Token?.AccessToken))
                return null;
    ...
    /// <summary>
    /// Google's <see cref="LocalServerCodeReceiver"/> waits on
    /// <c>HttpListener.GetContextAsync</c>, which often ignores
    /// <c>listener.Stop()</c> on macOS. Cancel via token + a dummy GET so the
    /// listener actually unblocks and the next login can bind a port.
    /// </summary>
    private sealed class CancellableLocalServerCodeReceiver : ICodeReceiver
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            using var unblock = cancellationToken.Register(() =>
            {
                _ = Task.Run(() => TryUnblockListener(RedirectUri));
            });

            return await _inner.ReceiveCodeAsync(url, cancellationToken)
                .WaitAsync(cancellationToken)
                .ConfigureAwait(false);
        }

        private static void TryUnblockListener(string redirectUri)
        {
            try
            {
                ...
                var probe = redirectUri.TrimEnd('/') + "/?error=access_denied";
                using var client = new HttpClient { Timeout = TimeSpan.FromSeconds(2) };
                client.GetAsync(probe).GetAwaiter().GetResult();
            }
            catch
            {
